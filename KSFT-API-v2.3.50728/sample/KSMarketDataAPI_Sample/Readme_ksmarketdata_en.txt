...
KSMarketDataAPI_Sample directory includes the following files
source file   :MdUserApi_Sample.cpp   
makefile of Linux enviroment:   makefile
project file of windows enviroment :MdUserApi_Sample.sln£¬MdUserApi_Sample.vcproj
help doc:       Readme_ksmarketdata.txt , Readme_ksmarketdata_en.txt(English) 

Part Two£ºFiles Function£º
MdUserApi_Sample.cpp  			implement the main functions of testing libksmarketdataapi.so dynamic library
makefile 			    				implement the compile progress of the test demo for linux platform
MdUserApi_Sample.sln£¬MdUserApi_Sample.vcproj		implement the compile progress of the test demo for windows platform
...
1, Compile Description
   ...
   3£©  Executable Files
	Run "make" or "make vtype=64r" will generate "MdUserApi_Sample" Executable file in run directory which used to test 64-bit 	libksmarketdataapi.so
        Run "make vtype=32r" will generate "MdUserApi_Sample" Executable file in run directory which used to test 32-bit libksmarketdataapi.so      
...
2£¬Run Description
   ...
   1£©Run Checking
      First make sure the existence of libksmarketdataapi.so and libkslkc64r.so£¨Release files include both 32 and 64 bit libksmarketdataapi.so,which uses the same file name, divided by the different directory "sdk/linux/linux32" and "sdk/linux/linux64"¡£ libkslkc32r.so is in sdk/linux/linux32 directory, while libkslkc64r.so is in sdk/linux/linux64 directory.£©
   ...
   4) Check Correctness of environment configuration
	...
	ldd  MdUserApi_Sample   : check the load stage of test demo 
...
1£¬Compile Description
   ...
   2£©How to Compile
   Use Visual Studio 2005 to open "MdUserApi_Sample.sln" file £¬then compile in the IDE.
   
   3£©Executable Files
   After compiling progress , you can find the executable file : MdUserApi_Sample.exe in the run directory .

2£¬Run Description
   First make sure run directory has these files: KSMarketDataAPI.dll,cpack.dll,lkcdll.dll
   Then you can execute the MdUserApi_Sample.exe in run directory.
   The correctness for running program also needs authorization document "KSInterB2C.lkc" which belongs to Sungard.
   Make sure the running directory has "KSInterB2C.lkc" file .You can connect hot line  for this file if you don't have it .
