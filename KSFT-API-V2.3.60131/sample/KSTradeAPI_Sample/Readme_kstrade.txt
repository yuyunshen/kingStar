...
一、文件说明
KSTradeAPI_Sample文件夹包含如下文件：
源文件   TraderApi_Sample.cpp
linux环境makefile文件   makefile
windows环境项目文件     TraderApi_Sample.sln，TraderApi_Sample.vcproj
帮助文件		Readme_kstrade.txt, Readme_kstrade_en.txt(英文帮助）

二、文件功能
TraderApi_Sample.cpp  			                        主要实现测试libkstradeapi.so动态库的基本功能
makefile 			    				主要实现在linux平台下，测试可执行程序的编译。
TraderApi_Sample.sln，TraderApi_Sample.vcproj     		主要实现windows平台下，测试可执行程序的编译。
...
1，编译说明
   ...
   3）  生成目标
	make 或 make vtype=64r 将在 KSTradeAPI_Sample 目录下，生成 TraderApi_Sample可执行文件，用于64位测试程序。
        make vtype=32r         将在 KSTradeAPI_Sample 目录下，生成 TraderApi_Sample可执行文件，用于32位测试程序。
...
2，运行说明
   ...
   1）运行检查
      测试代码的运行，需要确保动态库libkstradeapi.so,libkslkc64r.so（32位则是libkslkc32r.so）文件的存在。（发布物包括32位和64位的libkstradeapi.so，文件名相同，分别在sdk/linux/linux32/和   sdk/linux/linux64目录下。而libkslkc32r.so在sdk/linux/linux32/目录下，libkslkc64r.so在sdk/linux/linux64/目录下）
   ...
   4) 检查环境配置的正确性
	...
	ldd  TraderApi_Sample   检查测试程序对动态库的加载
      b)查看当前的动态库的版本信息
...
1，编译说明
   ...
   2）如何编译
   通过vs打开TraderApi_Sample.sln文件，编译即可。
   
   3）生成目标
   在KSTradeAPI_Sample目录下生成TraderApi_Sample.exe可执行文件。

2，运行说明
   确保run目录下有KSTradeAPI.dll,cpack.dll,lkcdll.dll
   在KSTradeAPI_Sample目录下执行KSTradeAPI_Sample.exe即可。
